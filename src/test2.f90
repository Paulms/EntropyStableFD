...
subroutine test2_run(run_ref, run_err)
  ...
  IF (run_ref) THEN
    ...
    names = ['x    ', 'REF  ']
    results(:,1) = xx
    CALL Entropy_Conservative(FORWARD_EULER, .TRUE., uu, N, Tend, dx, CFL, fluxEC, DiffMat, Cdt, 0.2_dp*dx, ZERO_FLUX) !ESC-0.2
    results(:,2) = uu
  ...
  IF (run_err) THEN
    ...
    DO i = 1, 4
      ...
      CALL Entropy_Conservative(FORWARD_EULER, .TRUE., uu, N, Tend, dx, CFL, fluxEC, DiffMat, Cdt, 0.2_dp*dx, ZERO_FLUX) !ESC
      error = cumpute_errors(reference(:,2), M, uu, N)
      results(3, i) = error
      uu = uinit; error = 0.0_dp
      CALL Entropy_NonConservative(FORWARD_EULER, .TRUE., uu, N, Tend, dx, CFL, fluxEC, KKN, Cdt, 0.2_dp*dx, ZERO_FLUX) !ESNC
      error = cumpute_errors(reference(:,2), M, uu, N)
      results(4, i) = error
      uu = uinit; error = 0.0_dp
  ...
  results(:,4) = uu
  uu = uinit
  CALL Entropy_NonConservative(FORWARD_EULER, .TRUE., uu, N, Tend, dx, CFL, fluxEC, KKN, Cdt, 0.2_dp*dx, ZERO_FLUX) !ESNC-0.2
  results(:,5) = uu
  CALL save_matrix(results, names, name, 0)
